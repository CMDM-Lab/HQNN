...
log_path = "./log_test_iso_nohit_refsis.txt"
...
def main():
    ...
    # 建立 SMILES 對應的頻譜 lookup 字典
    pred_dict = {row[0]: np.array(row[1:], dtype=float) for row in spectra_pred}
    ref_dict = {row[0]: np.array(row[1:], dtype=float) for row in spectra_ref}

    with open("log_test_iso_nohit.txt", 'r') as f:
        for line in f:
            parts = line.strip().split(',')
            try:
                smile1 = parts[0].strip()
                smile2 = parts[1].strip()

                if smile1 in pred_dict and smile2 in ref_dict:
                    spec1 = pred_dict[smile1]
                    spec2 = ref_dict[smile2]

                    sim = spectral_information_similarity(spec1, spec2, conv_matrix)
                    print(f"{smile1}, {smile2}, {sim}")

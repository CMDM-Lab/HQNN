...
log_path = "./log_test_iso_ffnn.txt"
...
def main():
    ...
    for pred_row, pred_smiles in zip(spectra_pred, smiles_pred):
        set1 = get_atom_counts(pred_smiles)
        ...
        for ref_row in spectra_ref:
            ...
            set2 = get_atom_counts(ref_smiles)
            if set1 != set2:
                continue

            if ref_smiles != pred_smiles:
                iso.append(ref_smiles)

            sim = spectral_information_similarity(pred_spec, ref_spec, conv_matrix)
            # sim_list.append((ref_smiles, sim)) 
            if sim > max_sim:
                max_sim = sim
                ans = ref_smiles

        print(f"{pred_smiles}, {ans}, {max_sim}")
        ...
        elif pred_smiles != ans and len(iso) >= 1:
            iso_nohit+=1

        # top5 = sorted(sim_list, key=lambda x: x[1], reverse=True)[:5]

        # if any(ref_smiles == pred_smiles for ref_smiles, _ in top5):
            # count_k += 1
        ...
        #     if score <= 1:
        #         print(f"{pred_smiles}, {ans}, {max_sim}, {score}, {sis_scores[pred_smiles]}")

    print(f"Top-{k} 命中數: {count_k}")
    print(f"iso命中數: {iso_hit}")
    print(f"iso未命中數: {iso_nohit}")

...
if __name__ == '__main__':
    ...
    for idx, predict in enumerate(predicts):
        ...
        # q1_tmse = tmse_df['TMSE'].quantile(0.25)
        # median_tmse = tmse_df['TMSE'].quantile(0.5)
        # q3_tmse = tmse_df['TMSE'].quantile(0.75)
        ...
        if predict.startswith('classical'):
            model_type = 'FFNN'
        elif predict.startswith('qh'):
            model_type = 'HQNN'
        elif predict.startswith('ensemble'):
            model_type = 'Ensemble'

        for tmse_value in tmse_df['TMSE']:
            results.append({
                'Group': group_id,
                'Model': model_type,
                'TMSE': tmse_value
        ...
        #
    results_df = pd.DataFrame(results)
    results_df.to_csv('./analysis/TMSE_detailed.csv', index=False)
